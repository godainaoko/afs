...
func mount(c *cli.Context) error {
	...
	if c.Bool("background") && os.Getenv("AFS_FOREGROUND") == "" {
		...
		if strings.HasPrefix(addr, sqliteScheme) {
			p := addr[len(sqliteScheme):]
			path2, err := filepath.Abs(p)
			if err == nil && path2 != p {
				for i, a := range os.Args {
					if a == addr {
	...
	if !c.Bool("no-usage-report") {
		...
		// NOTE: No need to report usage
	}
	mountMain(conf, m, store, c)
	return nil
}

func clientFlags() []cli.Flag {
	var defaultCacheDir = "/var/afsCache"
	switch runtime.GOOS {
	case "darwin":
		fallthrough
	case "windows":
		...
		defaultCacheDir = path.Join(homeDir, ".avefs", "cache")
	}
	return []cli.Flag{
		&cli.IntFlag{
			Name:  "get-timeout",
...
func mountFlags() *cli.Command {
	...
	cmd.Flags = append(cmd.Flags, mountFlag()...)
