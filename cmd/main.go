...
func main() {
	...
	if strings.HasSuffix(os.Args[0], "/mount.avefs") {
...
func handleSysMountArgs() ([]string, error) {
	...
	for _, option := range mountOptions {
		...
		for _, opt := range opts {
			...
			// The lower case option name is preferred, but if it's the same as the flag name, we also accept it
