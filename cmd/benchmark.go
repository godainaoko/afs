...
func benchmark(c *cli.Context) error {
	...
	dest = filepath.Join(dest, fmt.Sprintf("__avefs_benchmark_%d__", time.Now().UnixNano()))
	...
	if stats != nil {
		...
		st := func(n string) float64 {
			return stats2["avefs"+n] - stats["avefs"+n]
		}
		...
		fmt.Printf("Used: %.1fs, CPU: %.1f%%, MEM: %.1f MiB\n", st("uptime"),
			st("cpu_usage")*100/st("uptime"), stats2["avefs_memory"]/1024/1024)
	}
	return nil
}
