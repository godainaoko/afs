...
func newCacheStore(dir string, cacheSize int64, pendingPages int, config *Config, uploader func(key, path string)) *cacheStore {
	if config.CacheMode == 0 {
		config.CacheMode = 0600 // only the owner can read/write cache
	}
...
func (cache *cacheStore) createDir(dir string) {
	// who can read the cache should be able to access the directories and add a new file.
...
// flush cached block into the disk
...
func (cache *cacheStore) uploadStaging() {
	...
	for key, value := range cache.keys {
		...
		if cnt == 0 || lastValue.atime/60 > value.atime/60 ||
			lastValue.atime/60 == value.atime/60 && lastValue.size > value.size { // both sizes are < 0
			lastKey = key
			lastValue = value
		}
		cnt++
...
// hasMeta reports whether a path contains any of the magic characters
// recognized by Match.
