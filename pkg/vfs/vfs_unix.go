...
func StatFS(ctx Context, ino Ino) (st *Statfs, err int) {
    var totalSpace, availableSpace, iUsed, iAvail uint64
    _ = m.StatFS(ctx, &totalSpace, &availableSpace, &iUsed, &iAvail)
    var bsize uint64 = 0x10000
    blocks := totalSpace / bsize
    bavail := blocks - (totalSpace-availableSpace+bsize-1)/bsize
    ...
    st.Favail = iAvail
    logit(ctx, "statfs (%d): OK (%d,%d,%d,%d)", ino, totalSpace-availableSpace, availableSpace, iUsed, iAvail)
    return
}
