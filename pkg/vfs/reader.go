...
func (r *frange) String() string {
    return fmt.Sprintf("[%d,%d,%d)", r.off, r.len, r.end())
}
func (r *frange) end() uint64 {
    return r.off + r.len
}
func (r *frange) contain(p uint64) bool {
    return r.off < p && p < r.end()
}
func (r *frange) overlap(a *frange) bool {
    return a.off < r.end() && r.off < a.end()
}
func (r *frange) include(a *frange) bool {
    return r.off <= a.off && a.end() <= r.end()
}
...
func retryTime(tryCnt uint32) time.Duration {
    if tryCnt < 30 {
...
func (s *sliceReader) run() {
    ...
    err := f.r.m.Read(meta.Background, inode, idx, &chunks)
    f.Lock()
    ...
    if n == int(need) {
        ...
        s.currentPos = 0 // start again from the beginning
        err = syscall.EIO
        f.tried++
        _ = f.r.m.InvalidateChunkCache(meta.Background, inode, idx)
        if f.tried >= f.r.maxRetries {
            s.done(err, 0)
        } else {
            s.done(0, retryTime(f.tried))
        }
    }
}
...
func (f *fileReader) guessSession(block *frange) int {
    ...
    if idx == -1 {
        for i, ses := range f.sessions {
            ...
            nim := ses.lastOffset - bt
            if ses.lastOffset < bt {
                nim = 0
            }
            if nim <= block.off && block.off < ses.lastOffset && (closestOff == 0 || ses.lastOffset < closestOff) {
                idx = i
                closestOff = ses.lastOffset
            }
        }
...
func (f *fileReader) readAhead(block *frange) {
    f.visit(func(r *sliceReader) {
        if r.state.valid() && r.block.off <= block.off && r.block.end() > block.off {
            if r.state == READY && block.len > f.r.blockSize && r.block.off == block.off && r.block.off%f.r.blockSize == 0 {
                // the next block is ready, reduce readahead by a block
    ...
    if block.len > 0 && block.off < f.length && uint64(atomic.LoadInt64(&readBufferUsed)) < f.r.readAheadTotal {
        if block.len < f.r.blockSize {
            block.len += f.r.blockSize - block.end()%f.r.blockSize // align to the end of a block
        }
        f.newSlice(block)
        if block.len > 0 {
            f.readAhead(block)
