...
func (s *sliceWriter) prepareID(ctx meta.Context, retry bool) {
    ...
    for s.id == 0 {
        ...
        f.Lock()
        if st != 0 && !errors.Is(st, syscall.EIO) {
            s.err = st
            break
        }
        if !retry || st == 0 {
...
func (c *chunkWriter) commitThread() {
    ...
    f.Lock()
    defer f.Unlock()
    // the slices should be committed in the order that is created
    for len(c.slices) > 0 {
        s := c.slices[0]
        for !s.done {
            if s.notify.WaitWithTimeout(time.Millisecond*100) && !s.frozen && time.Since(s.started) > flushDuration*2 {
                s.frozen = true
                go s.flushData()
            }
        }
        ...
        f.Lock()
        if err != 0 {
            if !errors.Is(err, syscall.ENOENT) && !errors.Is(err, syscall.ENOSPC) {
...
// protected by the file
func (f *fileWriter) writeChunk(ctx meta.Context, idx uint32, off uint32, data []byte) syscall.Errno {
    c := f.findChunk(idx)
...
func (f *fileWriter) Write(ctx meta.Context, off uint64, data []byte) syscall.Errno {
    ...
    for len(data) > 0 {
        ...
        if st := f.writeChunk(ctx, idx, pos, data[:n]); st != 0 {
            return st
        }
        data = data[n:]
        idx++
        ...
        for _, f := range w.files {
            ...
            for i, c := range f.chunks {
                ...
                for j, s := range c.slices {
                    if !s.frozen && (now.Sub(s.started) > flushDuration || now.Sub(s.lastMod) > time.Second ||
